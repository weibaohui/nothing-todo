...
// 因为已经晋级过的 todo 不需要重复晋级, 专家视图里通常走 ExpertList 的降级路径。
//
// 为什么不直接走专家新建流程: 现实场景中, 用户往往先在一个 todo 上把 prompt /
// 验收标准 / 工作空间打磨好, 然后才意识到「我以后还要复用」。升级路径比从头新建
// 更自然, 也保留了原 todo 的全部历史 (执行记录、tag、调度配置)。
//
// 文件结构拆 3 块, 让每个组件函数体 < 30 行:
// - PromoteTriggerButton: 行内的晋级按钮 (presentational)
// - PromoteConfirmModal:  二次确认弹窗 (presentational)
// - PromoteToExpertButton: 拥有状态与副作用的容器, 组合前两者

import { useState } from 'react';
import { Button, Modal, App } from 'antd';
import { ExperimentOutlined } from '@ant-design/icons';
import { promoteTodoToExpert } from '@/utils/database/experts';
import { getAllTodos } from '@/utils/database/todos';
import { useApp } from '@/hooks/useApp';

interface PromoteToExpertButtonProps {
  todoId: number;
  todoTitle: string;
}
...
function PromoteModalBody({ todoTitle }: { todoTitle: string }) {
  return (
    <>
      <p style={{ marginTop: 0 }}>
        将 <b>「{todoTitle}」</b> 从「事项」提升为「专家」, 之后可在
        <b> 环路编排 (Loop Studio) </b>中作为可复用的执行单元被任意 loop 引用。
      </p>
      <p style={{ color: 'var(--color-text-secondary)' }}>晋级后典型用法:</p>
      <ul style={{ color: 'var(--color-text-secondary)', paddingLeft: 20, marginTop: 4 }}>
        <li>在 Loop Studio 新建 loop, 把此专家拖入阶段步骤</li>
        <li>被多个 loop 共享, 触发时并行 / 串行复用同一份 prompt 与配置</li>
      </ul>
      <p style={{ color: 'var(--color-warning)', fontSize: 12, marginBottom: 0 }}>
        提示: 此操作会保留 todo 的全部历史 (执行记录 / tag / 调度), 但
        <b> 晋级后此 todo 不再出现在「事项」过滤下</b>, 如需恢复可在「专家」管理页降级。
      </p>
    </>
  );
}
...
export function PromoteToExpertButton({ todoId, todoTitle }: PromoteToExpertButtonProps) {
  ...
  async function handleConfirm(): Promise<void> {
    setLoading(true);
    try {
      await promoteTodoToExpert(todoId);
      message.success(`「${todoTitle}」已晋级为专家`);
      setOpen(false);
